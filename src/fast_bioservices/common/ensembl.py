...
async def get_valid_ensembl_species(value: int | str | Taxon):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get("http://rest.ensembl.org/info/species", headers={"Content-Type": "application/json"})
        ...
        for species in response.json()["species"]:
            if taxon_value in {
                ...
                return species["name"]
        raise ValueError(f"{taxon_value} is not a valid ensembl species. Visit https://www.ensembl.org to get a valid species identifier")
